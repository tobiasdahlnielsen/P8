...
pacman::p_load(dbplyr,RSQLite,DBI,tidyverse,dplyr,magrittr,tictoc,beepr)
...
clean <- function(list,stock = "SPY",dir,close,DB){
  ...
  for(file in list){
    ...
    data <- data[(data$ex == "N"|data$ex == "T"|data$ex == "A"|data$ex == "Q"), ]  
    if(length(data[,1])==0){
    ...
    data <- data[!duplicated(data$utcsec), ]
    ...
    # Write to database.
    dbWriteTable(DB,name = stock,value = data,append = TRUE,overwrite = F)
    cat("File completed = ", file, "\n")
  }
}

Directory <- "D:/uni/P8/data/"
stock <- "SPY"
...
# table <- read.csv(paste0(Directory,stock,"/",list[1]),stringsAsFactors = F,sep=",") # %>% tail(1)

tic()
clean(list=list[301:length(list)],stock = stock,dir = Directory,close=tail(table,1),DB = DB)
toc()
